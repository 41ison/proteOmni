  ...
  tagList(
    ...
    selectInput(
      ...
      choices = c(
        ...
        "Mean AA-score distribution" = "plot_aa_score",
        "Peptide length distribution" = "plot_length",
        "Charge state distribution" = "plot_charge",
        "m/z error (exp − calc)" = "plot_mz_error",
        "Score vs mean AA score" = "plot_score_vs_aa",
        "Number of retained PSMs vs. score threshold" = "plot_elbow",
        "Retention time vs. m/z error" = "plot_rt_mz_error",
        "Retention time vs. PSM count" = "plot_rt_density",
        "Modifications summary" = "plot_mods",
        "Modified vs Unmodified PSMs" = "plot_mod_pie",
        "GRAVY (Grand Average of Hydropathy)" = "plot_gravy",
        "Isoelectric Point (pI) distribution" = "plot_pi",
        "Median score by peptide length" = "plot_score_length",
        "m/z error vs Casanovo score" = "plot_mz_score",
        "Amino acid frequencies" = "plot_aa_freq",
        "N-terminus SeqLogo" = "plot_nterm",
        "C-terminus SeqLogo" = "plot_cterm",
        "N:C-terminus co-occurrence probability" = "plot_cooccurrence"
      )
    ),
    actionButton(
      ns("run_plot"),
      ...
      tabPanel(
        ...
        fluidRow(
          box(
            ...
            width = 12,
            collapsible = TRUE,
            sp_wrap(ns("spi_main"), uiOutput(ns("dynamic_plot_ui")))
          )
        )
      ),
...
deNovo_server <- function(id) {
  moduleServer(id, function(input, output, session) {
    ...
    plot_cooccurrence_obj <- reactive({
      d <- data()
      req(nrow(d) > 0)
      aas <- c(
        "A",
