...
1. **Launch the Application**: Run the `Launch_proteOmni` file for your operating system (`.bat` for Windows, `.command` for macOS). Alternatively, open R/RStudio and run `shiny::runApp("proteOmni")`.
...
4. Click **"New"** and paste the folder path to your R `bin` directory. This path usually looks like:
   `C:\Program Files\R\R-4.x.x\bin` *(Replace `4.x.x` with your specific R version)*.
...
If the error points to a different package, replace `"promises"` with the package name shown in the error message. After installation, re-launch proteOmni using the `.bat` or `.command` file.
You can check the version of the installed package with:
